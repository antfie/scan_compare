...
func ParseAppIdFromScanInformation(urlOrAppId string) (int, error) {
	appId, err := strconv.Atoi(urlOrAppId)

	if err == nil {
		if appId <= 0 {
			return -1, errors.New("app ID must be positive")
		}
	...
	if !IsPlatformURL(urlOrAppId) {
		return -1, errors.New(reportPlatformUrlInvalid(urlOrAppId))
	}
